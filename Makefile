...
export TERRAFORM_PROVIDER_REPO := https://github.com/infoblox/terraform-provider-infoblox
...
pull-docs:
	@if [ ! -d "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)" ]; then \
		...
		git clone -c advice.detachedHead=false --depth 1 --filter=blob:none --branch "v$(TERRAFORM_PROVIDER_VERSION)" --sparse "$(TERRAFORM_PROVIDER_DOCS_REPO)" "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)"; \
	fi
...
#   You can check the basic implementation here: https://github.com/upbound/uptest/blob/main/internal/infobloxs/01-delete.yaml.tmpl.
